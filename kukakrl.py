#!/usr/bin/env python 
"""
Modified by Ahmad Saeed 2017, Fab Lab Egypt & Student at Zagazig University.

based on repetier.py (C) 2015 Marcus Littwin, Hot-World GmbH & Co. KG, repetier.com
...
class laser_gcode(inkex.Effect):
    ...
    def export_gcode(self,gcode):
        ...
        progName = self.options.file.replace(".src", "")
        f.write("DEF " + progName + "( )\n")
        f.write(";Program generated by 'Path To KRL' extension for 'Inkscape'\n")
        f.write(";on " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")) + "\n")
        f.write(";by " + self.options.usrname + "\n")
        f.write("BAS (#INITMOV,0)" + "\n")
        f.write("$BASE=" + self.options.base_co + ":{frame:X " + str(self.options.offx) + ",Y " + str(self.options.offy) + ",Z 0,A 0,B 0,C 0}\n" )
        f.write("$TOOL=" + self.options.tool_co + "\n")
        if (self.options.approx == 'C_DIS'): f.write("$APO.CDIS=" + str(self.options.approxval) + "\n" )
        ...
        f.write("$OV_PRO=100\n")
        f.write("BAS (#VEL_PTP,10)" + "\n")
        f.write("$VEL.CP=" + str(round(self.options.travel_speed,3)) +"\n" )
        f.write("PTP {A1 0,A2 -90,A3 90,A4 0,A5 0,A6 0}\n")
        f.write("PTP {A "+ str(self.options.oria) + ",B " + str(self.options.orib) + ",C "+ str(self.options.oric) +"}\n" )
        f.write("PTP {X 0,Y 0,Z " + str(self.options.travel_depth) +"}\n" )
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--username",                        action="store", type="string",          dest="usrname",                             default="KUKA User",                             help="Output directory")
        ...
        self.OptionParser.add_option("",   "--tool-speed",                      action="store", type="float",           dest="tool_speed",                          default="0.05",                         help="Tool speed (m/sec)")
        self.OptionParser.add_option("",   "--travel-speed",                    action="store", type="float",           dest="travel_speed",                        default="0.15",                         help="Travel speed (m/sec)")
        self.OptionParser.add_option("",   "--laser-depth",                     action="store", type="string",          dest="pass_depth",                          default="0",                            help="Depth of tool work (mm)")        
        self.OptionParser.add_option("",   "--travel-depth",                    action="store", type="int",             dest="travel_depth",                        default="10",                           help="Height of tool travel (mm)")
        self.OptionParser.add_option("",   "--base",                            action="store", type="string",          dest="base_co",                             default="$WORLD",                       help="Base Coordinate System")                
        self.OptionParser.add_option("",   "--tool",                            action="store", type="string",          dest="tool_co",                             default="$NULLFRAME",                   help="Tool Type")                
        self.OptionParser.add_option("",   "--approximation",                   action="store", type="string",          dest="approx",                              default="Distance",                     help="Approximation Type")                
        self.OptionParser.add_option("",   "--approx-value",                    action="store", type="float",           dest="approxval",                           default="0.5",                          help="Approximation Value")                
        self.OptionParser.add_option("",   "--ori-a",                           action="store", type="int",             dest="oria",                                default="180",                          help="Orientation Angle A")
        self.OptionParser.add_option("",   "--ori-b",                           action="store", type="int",             dest="orib",                                default="180",                          help="Orientation Angle B")
        self.OptionParser.add_option("",   "--ori-c",                           action="store", type="int",             dest="oric",                                default="0",                            help="Orientation Angle C")        
        self.OptionParser.add_option("",   "--offsetx",                         action="store", type="float",           dest="offx",                                default="0",                            help="Base Offset X (mm)")        
        self.OptionParser.add_option("",   "--offsety",                         action="store", type="float",           dest="offy",                                default="0",                            help="Base Offset Y (mm)")        
...
################################################################################
    def generate_gcode(self, curve, layer, depth):
        ...
        def c(c):
            ...
            for i in range(6):
                if c[i]!=None:
                    global strtX
                    global strtY
                    if i == 0: strtX = (round(c[i],4))
                    if i == 1: strtY = (round(c[i],4))
        ...
        lg, f =  'G00', "$VEL.CP=%f"%tool['penetration feed']                    ######### TOOL DOWN 
        current_a = 0
        ...
        #    Creating Gcode for curve between s=curve[i-1] and si=curve[i] start at s[0] end at s[4]=si[0]
            ...
            if s[1]    == 'move':
                g += "LIN {" + c(si[0]) + "} " + self.options.approx + "\n" # + tool['gcode before path'] + "\n"   ######### Between 2 vel.cp 
                lg = 'G00'
            elif s[1] == 'end':
                g += tool['gcode after path'] + "\n"
                lg = 'G00'
            elif s[1] == 'line':
                if lg=="G00": g += feed + "\n" + "LIN {Z " + self.options.pass_depth + "} " + self.options.approx + "\n"  ######### OPTIONAL TOOL DOWN
                g += "LIN {" + c(si[0]) + "} " + self.options.approx + "\n"
                lg = 'G01'
            elif s[1] == 'arc':
                ...
                if (r[0]**2 + r[1]**2)>.1:
                    ...
                    if abs(r1.mag()-r2.mag()) < 0.001 :
                        ...
                        cY = startY + j
                        R2 = pow(i,2) + pow(j,2)
                        strtAngle = math.atan2(startY - cY, startX - cX)
                        endAngle = math.atan2(endY - cY, endX - cX)
                        if strtAngle < 0: strtAngle += 2*pi
                        if endAngle < 0 : endAngle += 2*pi
                        if (s[3]<0):
                            if strtAngle < endAngle: strtAngle += 2*pi
                            auxAngle = strtAngle - abs (strtAngle - endAngle) / 2.0 
                        else:
                            if strtAngle > endAngle: endAngle += 2*pi
                            auxAngle = strtAngle + abs (strtAngle - endAngle) / 2.0 
                        if auxAngle > 2*pi: auxAngle -= 2*pi
                        elif auxAngle > pi: auxAngle -= 2*pi;
                        auxX = math.sqrt(R2) * math.cos(auxAngle) + cX;
                        auxY = math.sqrt(R2) * math.sin(auxAngle) + cY;

                        
                        g += (("CIRC {X "+ str(round(auxX,4)) + ",Y " + str(round(auxY,4)) + "},{")) 
                        g += c(si[0]) + "} " + self.options.approx + "\n"
                    else:
                        r = (r1.mag()+r2.mag())/2
                        g += ("G2" if s[3]<0 else "G3") + c(si[0]) + " R%f" % (r) + "\n"
                    lg = 'G02'
                else:
                    g += "LIN {" +c(si[0]) + "} " + self.options.approx + feed + "\n"
                    lg = 'G01'
        if si[1] == 'end':
            g += tool['gcode after path'] + "\n"
        return g
...
################################################################################
    def effect(self) :
        ...
        self.tools = {
            "name": "Laser Engraver",
            "id": "Laser Engraver",
            "penetration feed": round(self.options.tool_speed,3),
            "feed": round(self.options.tool_speed,3),
            "gcode before path": (""),
            "gcode after path": ("LIN {Z " + str(self.options.travel_depth) + "} "+ self.options.approx + "\n" + "$VEL.CP=" + str(round(self.options.travel_speed,3)) ),
        }
